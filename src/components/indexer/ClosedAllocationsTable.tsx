...
export function ClosedAllocationsTable({ allocations }: { allocations: ClosedAllocation[] }) {
  ...
  const current = Math.min(page, Math.max(0, Math.ceil(allocations.length / PAGE_SIZE) - 1));
  ...
  return (
    <>
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {allocations.slice(current * PAGE_SIZE, (current + 1) * PAGE_SIZE).map((alloc) => {
      ...
      {allocations.length > PAGE_SIZE && (
        <Pagination page={current} pageSize={PAGE_SIZE} totalItems={allocations.length} onPageChange={setPage} />
      )}
    </>
  );
}
