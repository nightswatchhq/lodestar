...
describe('rateLimit — enforcement', () => {
  it('allows up to the limit then blocks (429) within the window', async () => {
    ...
    const first = await rateLimit(ip, '/api/sql/query');
    ...
    const blocked = await rateLimit(ip, '/api/sql/query');
  ...
  it('isolates counters per IP', async () => {
    const a = freshIp();
    const b = freshIp();
    const cap = (await rateLimit(a, '/api/sql/query')).limit;
    for (let i = 1; i < cap; i++) await rateLimit(a, '/api/sql/query');
    const aBlocked = await rateLimit(a, '/api/sql/query');
    const bOk = await rateLimit(b, '/api/sql/query');
  ...
  it('isolates counters per tier (different paths do not share a bucket)', async () => {
    const ip = freshIp();
    for (let i = 0; i < 10; i++) await rateLimit(ip, '/api/sql/query'); // exhaust 10
    const chatBlocked = await rateLimit(ip, '/api/sql/query');
