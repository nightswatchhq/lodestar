...
| V1 | all | inside the release, after nuthatch and the nests deploy: the stack-top kittiwake and lodestar run locally against the production nests through a temporary read-only user, and the indexer page and directory inspected in a browser against real data, before kittiwake or lodestar deploy (no pre-release check can see views that exist only in a PR) | everything above |
